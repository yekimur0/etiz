...
/* harmony export */   Select: () => (/* binding */ M),
/* harmony export */   Settings: () => (/* binding */ P),
/* harmony export */   Store: () => (/* binding */ k),
...
const hoveredItem = document?.querySelectorAll('.hovered-item');
...
document.body.addEventListener('click', e => {
  let target = e.target;
  if (target.classList.contains('link-sublist')) {
    e.stopPropagation();
    let sublist = target.querySelector('.sublist');
    sublist.classList.toggle('vis');
  }
  if (target.classList.contains('questions__item')) initAccordion(target);
  if (target.dataset.careerToggler) initCareerAccordion(target);
...
const initAccordion = target => {
  target.classList.toggle('questions__item--active');
};
const initCareerAccordion = target => {
  const parent = target.closest('.career-content__item');
  parent.classList.toggle('career-content__item--active');
};
...
const showModal = target => {
  const id = target.dataset.openModal;
